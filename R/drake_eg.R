...
# create any personalised functions. 

# I think I actually don't have that many.
# But I need to go back and create them. I think this will help my organisation and thinking within the code
# even before drake does its thing with efficient code running
...
#######################################################################
cleancharacters = function(u){
  a = str_match_all(u, "[[:alnum:]|[:blank:]|[:punct:]]" )
  a[lengths(a$species) == 0] <- NA_character_
  
  b = character()

  for (i in 1:length(u) ){
  c = str_c(a[[i]], collapse = "")
  b =  append(b, c)
  }
...
# Turn Leftover cultivars and Leftovers into functions here
...
plan <- drake_plan(
  ##############################################
  cleaningscripts = list.files(full.names = T, recursive = T, pattern = ".*v1.R")
  
  lapply(cleaningscripts, source)
  #############################################
  # Loading the files
  # get all the individual files from their folders
  ...
  ############################################################
  #clean the special characters out of species names
  x$species = cleancharacters(x$species)
  
  ############################################################
  # Use taxize on unique species names to develop a database
  
  x1 = gnr_resolve(unique(x$species))
...
###############################################################
  ...
  # now to stick the manually collected data and the scraped together
